...
// const _privateKey = new Buffer('b6a03207128827eaae0d31d97a7a6243de31f2baf99eabd764e33389ecf436fc', 'hex');
const _privateKey = 'b6a03207128827eaae0d31d97a7a6243de31f2baf99eabd764e33389ecf436fc';
const _coinbase = web3.eth.coinbase;
var unackedAssetsHash = [];
var dispatcher = new events.EventEmitter();
var maxBlockReceiptUnavailable = 5;
var newBlockCB = null;
...
export function setNewBlockGenCB (cb){
  newBlockCB = cb;
}

export function transferCustomToken(contractAddress, senderEthAddress, senderPrivateKey, receiverEthAddress, quantity, callback){
...
export function transferCustomTokenEx(contractAddress, senderEthAddress, senderPrivateKey, receiverEthAddress, quantity, callback){
  var myadvancedtoken = myadvancedtokenContract.at(contractAddress);
...
export function getCustomTokenBalance(contractAddress, userAddress) {
...
export function monitorIssueAssets() {
	web3.eth.filter('latest').watch(function (err, blockHash) {
		...
		if (err) {
			...
			const len = unackedAssetsHash.length;
			for (let i = len - 1; i >= 0; i-- ) {
				let item = unackedAssetsHash[i];
				let receipt = web3.eth.getTransactionReceipt(item.hash);
				// let receipt = web3.eth.getTransactionReceipt(item.hash, function (err, receipt) {
				// 	if (err) console.log('txHash-to-txReceipt-err: ', err);
				// 	else console.log('txHash-to-txReceipt-Receipt:', receipt);
				// });
				...
				if(receipt) {
					...
					if (item.blockOffse < maxBlockReceiptUnavailable) {
...
export function issueAsset(userEthAddress, userPrivateKey, assetContract, callback){
...
export default {
	...
	monitorIssueAssets: monitorIssueAssets,
	getCustomTokenBalance: getCustomTokenBalance,
	transferCustomTokenEx: transferCustomTokenEx,
	transferCustomToken: transferCustomToken,
	setNewBlockGenCB: setNewBlockGenCB,
